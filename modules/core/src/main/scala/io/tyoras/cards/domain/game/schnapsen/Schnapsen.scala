...
private class SchnapsenImplem[F[_]](fsm: FinalStateMachine[F, GameState])(l: StructuredLogger[F])(using F: Sync[F]) extends Schnapsen[F]:
    ...
    case (s, restart: Restart) =>
      Logger[F].debug(s"Player ${restart.playerId} has asked to restart a new game") >>
        Clock[F].getZonedDateTimeUTC
          .flatMap(now => initGameRound(GameContext.reset(s.round.context, now))(logger))
          .map(newRound => EarlyGameForehandTurn(newRound))
    case (s, end: End) =>
  ...
  private def findWinner(fhCard: Card, dlCard: Card): InternalGameState[F, Player] = StateT { state =>
    val winner =
      if (dlCard.suit == fhCard.suit && dlCard.rank > fhCard.rank) || (dlCard.suit == state.trumpSuit) then state.dealer
      else state.forehand
    (state, winner).pure[F]
  }
  ...
  private def checkPlayer(expectedPlayer: Player, playerId: PlayerId): F[Unit] =
    if expectedPlayer.id == playerId then F.unit else F.raiseError(WrongPlayer)
