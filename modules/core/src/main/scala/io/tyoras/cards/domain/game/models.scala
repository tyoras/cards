...
sealed abstract class GameType[S : Encoder, I <: GameInput : Decoder](val label: String):
  type State = S
  type Input = I
...
object GameType:
  case object Schnapsen extends GameType[schnapsen.model.GameState, schnapsen.model.SchnapsenInput]("schnapsen")
  case object War       extends GameType[war.model.GameState, war.model.WarInput]("war")
...
trait ActiveGame[F[_], State, Input <: GameInput]:
  def gameType: GameType[State, Input]
...
object Game:
  final case class Existing[State](id: FUUID, createdAt: ZonedDateTime, updatedAt: ZonedDateTime, data: Data[State]) extends Game[State]:
    ...
    override def gameType: GameType[State, ?] = data.gameType
  ...
  final case class Data[State](gameType: GameType[State, ?], players: NonEmptyList[FUUID], state: State) extends Game[State]:
