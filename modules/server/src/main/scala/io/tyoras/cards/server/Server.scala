...
object Server:
  ...
  object HttpWsApp:
    def of[F[_] : Async](
      ...
      val corsMiddleware = httpConfig.corsAllowedOrigins.toSeq match
        case Nil            => CORS.policy // emtpy means all origins are allowed
        case allowedOrigins => CORS.policy.withAllowOriginHost(allowedOrigins.contains)
      val all          = first +: remaining
      val authRoutes   = authEndpoint.routes.pipe(corsMiddleware.apply)
      val authedRoutes = all.map(_.authedRoutes).reduceLeft(_ <+> _).pipe(authMiddleware).pipe(corsMiddleware.apply)
      val httpRoutes   = (all.map(_.routes).reduceLeft(_ <+> _) <+> authedRoutes).pipe(corsMiddleware.apply)
      val wsRoutes     = all.map(_.wsRoutes(wsBuilder)).reduceLeft(_ <+> _).pipe(corsMiddleware.apply)
      Router(
        "api"  -> httpRoutes,
        "auth" -> authRoutes,
        "ws"   -> wsRoutes
